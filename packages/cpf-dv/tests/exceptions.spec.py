"""Behavioral spec for the ``cpf-dv`` exception hierarchy.

Combines the shared cases of the JavaScript reference suite
(``js/packages/cpf-dv/tests/exceptions.spec.ts``) and the PHP reference suite
(``php/packages/cpf-dv/tests/Specs/Exceptions.spec.php``), following the rules
documented in ``AGENTS.md`` §5.

Python decisions (see ``AGENTS.md`` §8.4):
...
def describe_cpf_check_digits_type_error():
    def describe_when_instantiated_through_a_subclass():
        class _TestTypeError(CpfCheckDigitsTypeError):
            pass
        ...
        def it_is_an_instance_of_cpf_check_digits_type_error():
            error = _TestTypeError(123, "number", "string", "some error")

            assert isinstance(error, CpfCheckDigitsTypeError)
...
def describe_cpf_check_digits_input_type_error():
    def describe_when_instantiated():
        def it_is_an_instance_of_type_error():
            error = CpfCheckDigitsInputTypeError(123, "string")
        ...
        def it_has_the_correct_name():
            ...
            assert type(error).__name__ == "CpfCheckDigitsInputTypeError"
        ...
        def it_sets_the_expected_type_property():
            error = CpfCheckDigitsInputTypeError(123, "string or string[]")
...
def describe_cpf_check_digits_exception():
    def describe_when_instantiated_through_a_subclass():
        class _TestException(CpfCheckDigitsException):
            pass
        ...
        def it_is_an_instance_of_cpf_check_digits_exception():
            exception = _TestException("some error")

            assert isinstance(exception, CpfCheckDigitsException)
...
def describe_cpf_check_digits_input_length_exception():
    def describe_when_instantiated():
        def it_is_an_instance_of_exception():
            exception = CpfCheckDigitsInputLengthException("1.2.3.4.5", "12345", 12, 14)
        ...
        def it_has_the_correct_name():
            ...
            assert type(exception).__name__ == "CpfCheckDigitsInputLengthException"
...
def describe_cpf_check_digits_input_invalid_exception():
    def describe_when_instantiated():
        def it_is_an_instance_of_exception():
            exception = CpfCheckDigitsInputInvalidException(
        ...
        def it_has_the_correct_name():
            ...
            assert type(exception).__name__ == "CpfCheckDigitsInputInvalidException"
        ...
        def it_generates_a_message_describing_the_exception():
            ...
            assert str(exception) == 'CPF input "1.2.3.4.5" is invalid. repeated digits'
